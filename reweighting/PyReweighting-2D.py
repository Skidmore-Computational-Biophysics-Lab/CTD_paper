...
def reweight_CE(data,hist_min,binsX,discX,binsY,discY,dV,T,fit):
    ...
    hist_max_float = np.max(hist2)
    hist_max = hist_max_float.astype(int)
##    print np.max(hist2)
    ...
    nA_float = np.zeros((nbinsX,nbinsY)) # nA is equivalent to hist here
    nA = nA_float.astype(int)
    ...
    dV_mat_float = np.zeros((nbinsX,nbinsY,hist_max)) # matrix for storing dV of each assigned 
    dV_mat = dV_mat_float.astype(int)
...
#check that the indent is correct for lines below
...
#check that the indent is correct for lines above
